...
clave1 = (new Date().getDate() + new Date().getDay() + (new Date().getMonth()+1) ).toString(); // Clave dinamica cortez
...
//        }
        ...
        $.when(d1).done(function (v1){
            ...
            if (v1.status){
                ...
                rsv_solicitar('cuenta_anular',{mesa:v1.data.attr('id_mesa'), cuenta: v1.data.attr('cuenta'), motivo: motivo},function(datos){
                    cmp_cache = null;
                    //alert(orden.attr('id_mesa') +"."+ orden.attr('cuenta') +"."+ motivo);
                });
            }else{
                alert('Usario denegado. Envio cancelado' );
                   return;
            }
        });        
    });
    ...
    function adminPass(data){               
        $("#dialog-password").dialog({
            ...
            close: function () {
                ...
                $('#dialog-user-input').val('');
            },
            buttons: {
                Ok: function () {
                    var username = $('#dialog-user-input').val();
                    var meseroPassword = $('#dialog-password-input').val();
                                                            
                    if (!meseroPassword) {
                       d1.resolve({status:false,data:data});
                    } else {
                       meseroPassCallback(data,username,meseroPassword)
                    }
                    $( this ).dialog( "close" );
                }
            }
    ...
    function meseroPassCallback(data, username, meseroPassword){
        consulta="select ID_usuarios from usuarios where usuario='" + username + "' and disponible=1  AND clave = sha1('" + meseroPassword + "')";
                
        rsv_solicitar('consultarUtils', {consulta:consulta, consultaType:"1"}, function(datos){
            if (datos.aux !==""){
                d1.resolve({status:true,data:data});
            }else{
                alert('Usuario no autorizado para Anular. Ingresar password nuevamente.');
                adminPass(data);
            }
        }, false);         
    }    
    ...
    $('#pedidos').on('click','.cerrar_cuenta', function(){
        ...
        while (vuelto ===0){
            ...
            var apagar=Number(document.getElementById ('totalcuenta2'+orden.attr('cuenta')+'.'+orden.attr('id_mesa')).innerText.substring(1));
            vuelto = prompt('Ingrese monto del pago: $'+apagar);  // totalcuenta2
            
            if (isNumeric(vuelto) || vuelto !== null || vuelto.trim() !== ""  || vuelto.trim() >0 ) {
                if(vuelto>=apagar){
                    alert('Cambio:( $' + vuelto + '- $' + apagar + ')= $' + (vuelto-apagar).toFixed(2)); 
                }else{
                    alert('Favor Ingrese un monto mayor o igual al pago pendiente. Ingreso: $'+vuelto+' Debe: $' + apagar);
                    vuelto =0;
    ...
    $(document).on('mousedown','.imp_factura', function(event){        
        ...
        rsv_solicitar('cuenta',{ cuenta: orden.attr('cuenta'), facturacion: '1'},function(datos){
            for(x in datos.aux.pendientes)
            {
                ...
                $.post('http://localhost:40001', {xml:xml}, function(data){
                if(data==='ACK'){
                        alert('Factura Consumidor Enviada a impresion');
                    }else{
                        alert('Error enviado a impresion');
                    }
                }, 'text');
            }
       });
    ...
    $(document).on('mousedown','.imp_fiscal', function(event){        
        ...
        rsv_solicitar('cuenta',{ cuenta: orden.attr('cuenta'), facturacion: '1'},function(datos){
            for(var x in datos.aux.pendientes)
            {
                ...
                $.post('http://localhost:40001', {xml:xml}, function(data){
                    if(data==='ACK'){
                        alert('Credito Fiscal Enviado a impresion');
                    }else{
                        alert('Error enviado a impresion');
                    }
    ...
    $("#compras").click(function() {
        rsv_solicitar('producto_ingredientes_y_adicionales',{modo: 'inventario'}, function(datos){
            ...
            buffer += '<tr><td><input type="text" name="empresa" value="" />' +
                    '</td><td><input type="text" name="descripcion" value="" />'+
                    '</td><td><input type="text" name="precio" value="0.00" />'+
                    '</td><td><select name="via">'+
                        '<option value="caja">Caja</option>'+                        
                        '<option value="gerencia">Gerencia</option>'+
                        '<option value="otras">Otras compras</option></select>'+
                        '</td><td><button id="guardar_compra">Guardar</button></td></tr>';            
            buffer += '</table>';
            buffer += '</form>';
...
//    '<option value="Cheque">Cheque</option>'+
    ...
    $('#ver_total').click(function(){
        ...
        if ( prueba == null || prueba.trim() !== clave1 ) {
            alert('La clave de CorteZ ingresada es incorrecta!');
            Beep();
            return;
        }
        ...
        rsv_solicitar('cortez',{fecha: $('#fecha_caja').val()},function(datos){
            ...
            buffer += '<p>Total Prods. eliminados: $' + datos.aux.total_cancelado + ' - <span style="color:#666;">solo pedidos eliminados</span></p>';
            ...
            buffer += '<p>Total comprasG: $' + datos.aux.total_comprasG + ' - <span style="color:#666;">solo dinero gastado de Gerencia en compras</span></p>';
            buffer += '<p>Total comprasOtras: $' + datos.aux.total_comprasO + ' - <span style="color:#666;">solo dinero de otras compras</span></p>';
            ...
            buffer += '<p style="color:yellow;">Total compras Caja a cuadrar: $' + datos.aux.total_compras_cuadrar + ' - <span style="color:#666;">solo compras desde el último corte Z</span></p>';
            ...
            if (parseFloat(datos.aux.total_pendiente) > 0.00) {
                ...
                buffer += '<tr><td>Total comprasG:</td><td><input id="total_comprasG" name="total_comprasG" readonly="readonly" type="text" value="'+datos.aux.total_comprasG_cuadrar+'" /></td></tr>';
                buffer += '<tr><td>Total comprasO:</td><td><input id="total_comprasO" name="total_comprasO" readonly="readonly" type="text" value="'+datos.aux.total_comprasO_cuadrar+'" /></td></tr>';
            ...
            buffer += '<tr><th>Empresa</th><th>Descripción</th><th>Fecha</th><th>Precio</th><th>Via</th></tr>';
            for(compra in datos.aux.compras)
            {
                buffer += '<tr >'+
                        '<td class="shrink">'+datos.aux.compras[compra].empresa+'</td><td class="shrink">'+
                        datos.aux.compras[compra].descripcion+'</td><td class="shrink">'+
                        datos.aux.compras[compra].fechatiempo+'</td><td class="shrink">$'+
                        datos.aux.compras[compra].precio+'</td><td class="shrink">'+
                        datos.aux.compras[compra].via+'</td></tr>';
            }
            buffer += '</table>';
            buffer += '</div>';
            buffer += '</td>';
    ...
    $(document).on('click',"#cortar", function(){  
        ...
        if(!$.isNumeric($("#total_a_cuadrar").val()))
        {
           alert('CorteZ necesita un Total a cuadrar.');
            return; 
        }                
        
        rsv_solicitar('cortez',{imprimiranulaciones: "0"}, function(){});
        rsv_solicitar('cortez',{ cortar: true, datos: $("#frm_cortez").serialize() },function(datos){
            ...
            if (ID_corte > 0)
            {
                ...
                //rsv_solicitar('cortez',{imprimir: ID_corte});
...
//                corte.append('<div>'+$("#contenedor_compras").html()+'</div>');
    ...
    $('#historial_cortez').click(function(){
        rsv_solicitar('cortez',{historial: true},function(datos){
            ...
            buffer += '<tr><th>Fecha</th><th>Total</th><th>Diferencia 1<th>Diferencia 2</th>'+
                    '<th>Efectivo</th><th>POS</th><th>Compras</th><th>ComprasG</th><th>ComprasO</th><th>Caja</th></tr>';
            for(y in datos.aux.historial)
            {
                buffer += '<tr ' + (datos.aux.historial[y].sospechoso == '1' ? 'style="background-color:red"' : '') +' >' + 
                        '<td>'+ datos.aux.historial[y].fechatiempo + '</td>' + 
                        '<td>'+ datos.aux.historial[y].total_a_cuadrar + '</td>'  + 
                        '<td>'+ datos.aux.historial[y].total_diferencia2 + '</td>' + 
                        '<td>'+ datos.aux.historial[y].total_diferencia + '</td>' + 
                        '<td>'+ datos.aux.historial[y].total_efectivo + '</td>' + 
                        '<td>'+ datos.aux.historial[y].total_pos + '</td>' + 
                        '<td>'+ datos.aux.historial[y].total_compras + '</td>' + 
                        '<td>'+ datos.aux.historial[y].total_comprasG + '</td>' +
                        '<td>'+ datos.aux.historial[y].total_comprasO + '</td>' +
                        '<td>'+ datos.aux.historial[y].total_caja + '</td>' + 
    ...
    $('#cortes').click(function(){
        ...
        window.location = window.location+'?TPL=cortes';
