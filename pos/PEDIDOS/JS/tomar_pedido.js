...
function reiniciarInterfaz() {
    ...
    _b_orden = [];
    ID_mesero_busqueda = '';
    mostrar_grupo_productos("1,0,2,4,5,6,7");
    miniResumenOrden();
    ResumenOrden();
}
...
function ResumenOrden()
{
    ...
    if (_orden.length == 0)
    {
        $("#scroller").html('<p  style="color:white;font-weight: bold;">No hay ningún pedido agregado</p>');
        return;
    }
    
    buffer += '<h1 style="color:white;font-weight: bold;">Resumen de la orden</h1>';
    
    buffer += '<table style="color:blue;font-weight:bold;" class="estandar ancha bordes zebra" id="seleccion_producto">';
    for (x in _orden)
    {
        var adicionales = '';
        ...
        buffer += '<td><div >' + _orden[x].detalle + '</div>'+ 
                '<div>' + adicionales + 
                '</div><div>' + quitar + '</div></td>';
...
/// **************  MESERO                        
    ...
    function consultarComanda(ID_mesa, ID_mesero, orden, mesaAbierta, ID_comandaA){
        ID_comanda=0;
        consulta="";
        ...
        if (!mesaAbierta){ //no hay abierta
            ...
            if (ID_comanda>0 ){
                consulta="select id_cuenta from cuentas where id_comanda='" + ID_comanda + "'";        
                rsv_solicitar('consultarUtils', {consulta:consulta, consultaType:"1"}, function(datos){

                    if (datos.aux ===""){

                        rsv_solicitar('ingresar_orden',{mesa: ID_mesa, mesero: ID_mesero, orden: orden, comanda: ID_comanda}, function(){
                          reiniciarInterfaz();
                          $('#info_principal').html('<div style="color:white;font-size:14px;font-weight:bold;text-align:center;">ORDEN ENVIADA</div>');
                        });
                        ...
                        lcuentas="";
                         for (x in datos.aux)
                          {
                           lcuentas+= datos.aux[x].id_cuenta+", ";             
                          }
                          alert ('Comanda repetida para Cuenta#: ' + lcuentas);
                          consultarComanda(ID_mesa, ID_mesero, _orden, mesaAbierta,ID_comandaA);
                    }
                }, false);
            }else{
                alert('Número de comanda invalido.' + ID_comanda);
                consultarComanda(ID_mesa, ID_mesero, _orden, mesaAbierta,ID_comandaA);
            }
        }else{ // hay abierta
            rsv_solicitar('ingresar_orden',{mesa: ID_mesa, mesero: ID_mesero, orden: orden, comanda: ID_comandaA}, function(){
                reiniciarInterfaz();
                $('#info_principal').html('<div style="color:white;font-size:14px;font-weight:bold;text-align:center;">ORDEN ENVIADA</div>');
            });
